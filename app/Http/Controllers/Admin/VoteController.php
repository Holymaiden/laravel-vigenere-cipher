...
{
    protected $voteContract,  $response;

    public function __construct(VoteContract $voteContract)
    {
        $this->title = "votes";
        $this->voteContract = $voteContract;
        // $this->middleware("votes:{$this->title}");
        $this->response = [
    ...
    public function data(Request $request)
    {
        try {
            ...
            $data = Vote::when($request->input('cari'), function ($query) use ($request) {
    ...
    {
        try {
            $data = $this->voteContract->store($request);
    ...
    {
        try {
            $data = $this->voteContract->show($id);
            return response()->json($data);
        } catch (\Exception $e) {
            $this->response['message'] = $e->getMessage() . ' in file :' . $e->getFile() . ' line: ' . $e->getLine();
            return view('errors.message', ['message' => $this->response]);
    ...
    {
        try {
            $data = $this->voteContract->update($request, $id);
            return response()->json($data);
        } catch (\Exception $e) {
            $this->response['message'] = $e->getMessage() . ' in file :' . $e->getFile() . ' line: ' . $e->getLine();
            return view('errors.message', ['message' => $this->response]);
    ...
    {
        try {
            $data = $this->voteContract->delete($id);
            return response()->json($data);
        } catch (\Exception $e) {
            $this->response['message'] = $e->getMessage() . ' in file :' . $e->getFile() . ' line: ' . $e->getLine();
            return view('errors.message', ['message' => $this->response]);
